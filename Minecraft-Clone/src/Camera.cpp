...
glm::mat4 Camera::GetViewMatrix()
{
    ...
    glm::vec3 yOffset(0.0f, 0.5f, 0.0f);
    return glm::lookAt(transform->position + yOffset, transform->position + yOffset + transform->front, transform->up);
}

//Camera::Camera(glm::vec3 position, glm::vec3 up, float yaw, float pitch)
//    :m_cameraPos(position), m_front(glm::vec3(0.0f, 0.0f, -1.0f)), m_up(up), m_right(glm::vec3(1.0f, 0.0f, 0.0f)), m_worldUp(up),
//    m_yaw(yaw), m_pitch(pitch), m_movementSpeed(10.0f), m_mouseSensitivity(0.1f)
//{
//    UpdateCameraVectors();
//}
